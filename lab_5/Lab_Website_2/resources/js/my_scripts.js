...
*/
	function viewStudentStats(id, num){
		if(num==0){
...
*/
	function loadStatsPage(){
		...
		var wins=0;
		var losses=0;
		for (rc=2;rc<table.rows.length;rc++){
			counter=0;
			for(cc=2;cc<table.rows[rc].cells.length;cc++){
				if (counter==0){
					cellVal1=table.rows[rc].cells[cc].innerHTML;
				}
				else if (counter==1){
					cellVal2=table.rows[rc].cells[cc].innerHTML;
				}
				else{
					if(cellVal1<cellVal2){
						table.rows[rc].cells[cc].innerHTML=cellVal1;
						wins++;
					}
					else{
						table.rows[rc].cells[cc].innerHTML=cellVal2;
						losses++;
					}
				}
				counter++;
...
function loadPlayersPage(){
 	...
 	for (var player_i=0;player_i<players.length;player_i++){
 		txt += '<a class="dropdown-item" href="#" onclick="switchPlayers('+ player_i +')">' + players[player_i].name + '</a>';
 	}
 	document.getElementById("player_selector").innerHTML = txt;
